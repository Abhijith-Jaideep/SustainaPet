...
class _QuestScreenState extends State<QuestScreen> {
  ...
  void _setupWeeklyReset() {
    ...
    _weeklyResetTimer = Timer(duration, () async {
      await _assignRandom(); // 每周重置
      _setupWeeklyReset(); // 递归下一周
    });
  }
  ...
  Future<void> _refreshQuest(UserQuestDto uq, {bool completed = true}) async {
    if (_userId == null) return;
    final uid = _userId!;
    final updated = await api.getUserQuests(
      uid,
  ...
  Future<void> _complete(UserQuestDto uq) async {
    ...
    setState(() => uq.iscompleted = true); // Optimistic UI for faster response

    try {
      final res = await api.completeUserQuest(uq.userquestid, moodDelta: uq.moodDelta);
      final event = res['event'] as Map<String, dynamic>?;
      ...
      // 单独刷新已完成列表
      await _refreshQuest(uq, completed: true);
            ...
            children: [
              ...
              Row(
                children: [
                  const Icon(Icons.eco_rounded, color: Colors.green),
                  const SizedBox(width: 8),
                  Text('CO₂ saved: ${savedAbs.toStringAsFixed(3)} kg'),
                ],
              ),
              const SizedBox(height: 8),
              Row(
                children: [
                  const Icon(Icons.sentiment_satisfied, color: Colors.amber),
                  const SizedBox(width: 8),
                  Text('Mood +${uq.moodDelta}'),
                ],
              ),
              if (uq.quest.details.isNotEmpty) ...[
                const SizedBox(height: 12),
                Text('Details: ${uq.quest.details}', style: const TextStyle(fontSize: 12)),
              ],
            ],
          ),
          actions: [
      ...
      if (!mounted) return;
      setState(() => uq.iscompleted = false); // revert
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Completion failed: $e')),
      );
    }
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            _badge('${quest.reward} pts', Colors.green),
                            const SizedBox(width: 8),
                            _badge('${quest.emissions.abs().toStringAsFixed(3)} kg CO₂',
                                Colors.teal),
                          ],
                        ),
                        if (widget.uq.completeddate != null) ...[
                        ...
                        if (quest.moodDelta > 0) ...[
                          const SizedBox(height: 4),
                          Text(
                            'Mood impact: +${quest.moodDelta}',
...
void main() {
  runApp(const MaterialApp(home: QuestScreen()));
}
